                            ...
                            <tbody>
                                @foreach ($collection as $item)
                                    <tr>
                                        ...
                                        <td>
                                            <div class="d-flex">
                                                <a href="{{ url('admin/StuffAssignforProject/' . $item->id . '/edit') }}"
                                                    class="btn btn-success me-2">Edit</a>
                                                <a
                                                    href="{{ url('admin/StuffAssignforProject/' . $item->id) }}"class="btn btn-info me-2">show</a>
                                                <form action="{{ url('admin/StuffAssignforProject/' . $item->id) }}"
                                                    method="post">
                                                    {{ method_field('Delete') }}
                                                    {{ csrf_field() }}
